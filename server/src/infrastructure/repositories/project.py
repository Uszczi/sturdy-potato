...
class ProjectRepository:
    ...
    async def list_for_user(self, user_id: int) -> list[ProjectSchema]:
    ...
    async def get_for_user(self, user_id: int, pk: int) -> ProjectSchema | None:
    ...
    async def create_for_user(
        ...
        created = await self.get_for_user(user_id, project.id)
    ...
    async def reorder_for_user(self, user_id: int, ordered_ids: list[int]) -> bool:
        ...
        projects = list(await self._session.scalars(statement))
    ...
    async def update(
        self, user_id: int, pk: int, data: Mapping[str, Any]
    ) -> ProjectSchema | None:
        project = await self._get_orm(user_id, pk)
        if project is None:
            return None
        ...
        self._session.add(project)
        await self._session.commit()
        return await self.get_for_user(user_id, pk)

    async def delete(self, user_id: int, pk: int) -> bool:
        project = await self._get_orm(user_id, pk)
        if project is None:
            return False
